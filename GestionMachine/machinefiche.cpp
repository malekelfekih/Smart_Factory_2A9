...
int Machinefiche :: getnum_modele()
{return num_modele_fiche ; }
...
bool Machinefiche::verifiernum_modele(int num_modele_fiche)
{
    QSqlQuery  query;
        bool test=false;
            query.prepare("Select * FROM MACHINE_FICHE where num_modele_fiche=:num_modele_fiche ");

             query.bindValue(":num_modele_fiche",num_modele_fiche);
...
QSqlQueryModel *Machinefiche::chercher1(int test,QString text)
{
    QSqlQuery query;
       ...
       if(test==2)
           {
            query.prepare("SELECT * FROM MACHINE_FICHE where AGE like '"+text+"'");
                query.exec();
                model->setQuery(query);
        }
       if(test==3)
           {
            query.prepare("SELECT * FROM MACHINE_FICHE where ETAT like '"+text+"'");
                query.exec();
                model->setQuery(query);
        }
